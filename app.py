...
from evolution_memory import EvolutionMemoryModel, make_stream, THEMES
...
# ──────────────────────────────────────────────────────
# LOAD PRE-TRAINED WEIGHTS  (instant — no training wait)
# ──────────────────────────────────────────────────────
LOSS_HISTORY = {
    "t": [0.7120, 0.0145, 0.0030, 0.0005, 0.0004, 0.0004, 0.0003, 0.0002, 0.0002, 0.0001],
    "v": [0.0238, 0.0013, 0.0002, 0.0001, 0.0001, 0.0001, 0.0001, 0.0000, 0.0000, 0.0000],
}

@st.cache_resource(show_spinner=False)
def load_model():
    model = EvolutionMemoryModel(num_classes=4)
    weights_path = os.path.join(os.path.dirname(__file__), "decoder_weights.pt")
...
# INFERENCE
# ──────────────────────────────────────────────────────
def run_pipeline(theme_id: int):
    stream     = make_stream(theme_id)
    stream_in  = stream.unsqueeze(0)
    with torch.no_grad():
        logits = model(stream_in)
        probs  = F.softmax(logits, dim=1).squeeze(0).numpy()
    pred_id   = int(np.argmax(probs))
    return stream, pred_id, THEMES[pred_id]["caption"], THEMES[theme_id]["caption"], probs, pred_id == theme_id
...
def plot_confidence(probs, pred_id):
    ...
    for bar, p in zip(bars, probs):
        ax.text(bar.get_width()+0.01, bar.get_y()+bar.get_height()/2,
                f"{p:.3f}", va="center", ha="left", fontsize=8, color="#aaa", fontfamily="monospace")
    ax.set_xlim(0, 1.15)
...
def plot_loss():
    t_hist = LOSS_HISTORY["t"]
    v_hist = LOSS_HISTORY["v"]
    ...
    for txt in legend.get_texts(): txt.set_color("#888")
    plt.tight_layout(pad=0.6)
    return fig
...
st.markdown("""
<div class="stat-row">
    <div class="stat-pill"><div class="sk">Encoder</div><div class="sv">ColorEnc/768</div></div>
    ...
    <div class="stat-pill"><div class="sk">Final Train Loss</div><div class="sv">0.0001</div></div>
    <div class="stat-pill"><div class="sk">Final Val Loss</div><div class="sv">0.0000</div></div>
</div>
""", unsafe_allow_html=True)
...
with left:
    ...
    st.pyplot(plot_loss(), use_container_width=True)
...
with right:
    ...
    st.markdown(
        ...
        "(Encoder → Episodic Memory Bank → Evolutionary Selector → Decoder), "
        "and produces a natural-language caption."
    )

    if st.button("RUN EVO-MEM PIPELINE"):
        with st.spinner("Running pipeline..."):
            stream, pred_id, pred_cap, truth_cap, probs, correct = run_pipeline(selected_id)
